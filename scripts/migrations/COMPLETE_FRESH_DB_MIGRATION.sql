-- =============================================
-- COMPLETE FRESH DATABASE MIGRATION
-- =============================================
-- This script creates ALL tables from scratch AND adds missing columns
-- Use this if you're starting fresh or missing base tables
-- =============================================
...
-- =============================================
-- STEP 2: Create Base Tables (if not exist)
-- =============================================
...
-- Table: vark_modules (with ALL columns including new ones)
CREATE TABLE IF NOT EXISTS public.vark_modules (
  ...
  prerequisite_module_id UUID REFERENCES public.vark_modules(id) ON DELETE SET NULL,
...
-- Table: vark_module_sections (with ALL columns including new ones)
...
-- Table: student_module_submissions
...
-- =============================================
-- STEP 3: Add Missing Columns (if tables existed before)
-- =============================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'vark_modules' AND column_name = 'module_metadata') THEN
        ALTER TABLE public.vark_modules ADD COLUMN module_metadata JSONB DEFAULT '{}';
    END IF;
    
    -- json_content_url
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'vark_modules' AND column_name = 'json_content_url') THEN
        ALTER TABLE public.vark_modules ADD COLUMN json_content_url TEXT;
    END IF;
    
    -- content_summary
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'vark_modules' AND column_name = 'content_summary') THEN
        ALTER TABLE public.vark_modules ADD COLUMN content_summary JSONB DEFAULT '{}';
    END IF;
    
    -- target_class_id
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'vark_modules' AND column_name = 'target_class_id') THEN
        ALTER TABLE public.vark_modules ADD COLUMN target_class_id UUID REFERENCES public.classes(id) ON DELETE SET NULL;
    END IF;
    
    -- target_learning_styles
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'vark_modules' AND column_name = 'target_learning_styles') THEN
        ALTER TABLE public.vark_modules ADD COLUMN target_learning_styles JSONB DEFAULT '[]'::jsonb;
    END IF;
    
    -- prerequisite_module_id
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'vark_modules' AND column_name = 'prerequisite_module_id') THEN
        ALTER TABLE public.vark_modules ADD COLUMN prerequisite_module_id UUID REFERENCES public.vark_modules(id) ON DELETE SET NULL;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'vark_module_sections' AND column_name = 'learning_style_tags') THEN
        ALTER TABLE public.vark_module_sections ADD COLUMN learning_style_tags JSONB DEFAULT '[]';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'vark_module_sections' AND column_name = 'interactive_elements') THEN
        ALTER TABLE public.vark_module_sections ADD COLUMN interactive_elements JSONB DEFAULT '[]';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'vark_module_sections' AND column_name = 'metadata') THEN
...
-- First, check if columns have wrong type and fix them
DO $$
BEGIN
    ...
    IF EXISTS (
        ...
        ALTER TABLE public.vark_modules ALTER COLUMN content_summary TYPE JSONB USING content_summary::jsonb;
    END IF;
END $$;

-- Now update NULL values
...
COMMENT ON COLUMN public.vark_modules.module_metadata IS 'Additional metadata: content standards, competencies, key concepts, vocabulary';
COMMENT ON COLUMN public.vark_modules.target_class_id IS 'Reference to the target class for this module';
COMMENT ON COLUMN public.vark_modules.target_learning_styles IS 'Array of learning styles this module targets';
COMMENT ON COLUMN public.vark_modules.prerequisite_module_id IS 'Reference to a prerequisite module';
COMMENT ON COLUMN public.vark_modules.json_content_url IS 'URL to full module content in storage';
COMMENT ON COLUMN public.vark_modules.content_summary IS 'Lightweight summary of module content';
...
BEGIN
  ...
  RAISE NOTICE '  ✓ Created/verified all base tables';
  RAISE NOTICE '  ✓ Added all missing columns';
  RAISE NOTICE '  ✓ Created all indexes';
  RAISE NOTICE '  ✓ Set up RLS policies';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 Next Steps:';
  RAISE NOTICE '  1. Create storage buckets: module-images, module-content';
  RAISE NOTICE '  2. Test: npm run dev';
